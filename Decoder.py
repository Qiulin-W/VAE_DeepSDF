...
class Decoder:
    def __init__(self, net, weight_decay=0.0, dropout_keep_prob=0.8, batchnorm=True,
        ...
        :param net: is the input network. A tensor of size [6
            ...
            with slim.arg_scope([slim.fully_connected], weights_initializer=initializer,
                ...
                with slim.arg_scope([slim.batch_norm]):
                    ...
                    with slim.arg_scope([slim.dropout], is_training=is_training,
                        ...
                        self.input = net
                        self.end_point = slim.fully_connected(net, 512, activation_fn=tf.nn.relu)
                        self.end_point = slim.fully_connected(self.end_point, 512, activation_fn=tf.nn.relu)
                        self.end_point = slim.fully_connected(self.end_point, 512, activation_fn=tf.nn.relu)
                        self.end_point = slim.fully_connected(self.end_point, 512, activation_fn=tf.nn.relu)
                        self.mid_point = tf.concat([self.end_point, self.input], 1)
                        self.end_point = slim.fully_connected(self.mid_point, 512, activation_fn=tf.nn.relu)
                        self.end_point = slim.fully_connected(self.end_point, 512, activation_fn=tf.nn.relu)
                        self.end_point = slim.fully_connected(self.end_point, 512, activation_fn=tf.nn.relu)
                        self.end_point = slim.fully_connected(net, 1, activation_fn=tf.nn.tanh)
...
if __name__ == '__main__':
    ...
    x = tf.placeholder(tf.float32, [None, 1027])  # 1027 = 1024 latent variables + 3 coordinates
    # Class instantiation
