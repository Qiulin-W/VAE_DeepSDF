            ...
            with slim.arg_scope([slim.fully_connected], weights_initializer=initializer,
                ...
                with slim.arg_scope([slim.batch_norm]):
                    ...
                    with slim.arg_scope([slim.dropout], is_training=is_training,
                        ...
                        self.end_point = slim.fully_connected(net, 128, activation_fn=tf.nn.relu)
                        self.end_point = slim.fully_connected(self.end_point, 512, activation_fn=tf.nn.relu)
                        self.end_point = slim.fully_connected(self.end_point, 256, activation_fn=tf.nn.relu)
                        self.end_point = slim.fully_connected(self.end_point, 256, activation_fn=tf.nn.relu)
                        self.mid_point = tf.concat([self.end_point, self.input], 2)  # [?, K, 256+dim_in]
                        self.end_point = slim.fully_connected(self.end_point, 256, activation_fn=tf.nn.relu)
                        self.end_point = slim.fully_connected(self.end_point, 256, activation_fn=tf.nn.relu)
                        self.end_point = slim.fully_connected(self.end_point, 128, activation_fn=tf.nn.relu)
                        self.end_point = slim.fully_connected(self.end_point, 1, activation_fn=tf.tanh)  # [?, K, 1]
...
if __name__ == '__main__':
    ...
    latent_dim = 128
    x = tf.placeholder(tf.float32, [None, 32768, latent_dim+3])  # 2051 latent variables + 3 coordinates
    # Class instantiation
    print("Building Decoder...")
    decoder = Decoder(x)
    # Success message
    print("Decoder is built successfully.")
    print(decoder.mid_point.shape, decoder.end_point.shape)
